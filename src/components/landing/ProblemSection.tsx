...
const problems = [
  ...
  {
    icon: AlertCircle,
    text: "Usa apps diferentes e se perde",
  },
  {
    icon: HelpCircle,
    text: "Nunca sabe seu patrimônio total real",
  },
];

const ProblemSection = () => {
  return (
    <section className="pt-12 pb-24 relative overflow-hidden bg-card/30">
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="space-y-6 mb-12">
            {problems.map((problem, index) => (
              <div
                key={index}
                className="flex items-center gap-4 glass-card p-6 rounded-xl border border-destructive/20 hover:border-destructive/40 transition-colors"
              >
                <div className="w-12 h-12 rounded-xl bg-destructive/20 flex items-center justify-center flex-shrink-0">
                  <problem.icon className="w-6 h-6 text-destructive" />
                </div>
                <p className="text-lg md:text-xl text-foreground font-medium text-left">
                  {problem.text}
                </p>
              </div>
            ))}
